...
export default function Chattes() {
  const [chats, setChats] = useState([]);
  const [addMore, setAddMore] = useState(false);

  const { userData,status } = useSelector((state : any) => state?.auth);
  ...
  if(status){
    const unSub = onSnapshot(
      ...
      async (res) => {
        ...
        const items = res?.data()[0]?.chats;
  
        const promises = items.map(async (item) => {
          const userDocRef = doc(db, "users", item.recivedId);
        ...
        setChats(ChatData.sort((a,b)=> a.updateAt = b.updateAt))
      }
      ...
      <div className="flex h-screen antialiased text-gray-800">
        <div className="flex flex-row h-full w-full overflow-x-hidden">
          ...
          <div className="flex flex-col flex-auto h-full p-6">
            <div className="flex flex-col flex-auto flex-shrink-0 rounded-2xl bg-zinc-800 h-full p-4">
